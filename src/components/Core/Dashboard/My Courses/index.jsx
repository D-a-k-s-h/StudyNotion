...
const MyCourses = () => {
  ...
  return (
    <div className='flex flex-col gap-10 text-richblack-5 overflow-x-hidden'>
        <div className='w-full flex flex-col gap-10 md:gap-0 md:flex-row justify-between'>
            <p className='text-3xl'>My Courses</p>
            <button onClick={() => navigate("/dashboard/add-course")} type='button' className='w-fit flex items-center gap-1 text-richblack-900 bg-yellow-50 rounded-md p-2'>Add Course<FaPlus/></button>
        </div>
                        ...
                        <Tr>
                            ...
                            courses.map((course,index) => (
                                <Tr key={index} className='flex gap-10 p-2 justify-center'>
                                    <Td className='md:w-[70%] md:flex items-center gap-4'>
                                        <img src={course?.thumbnail} alt='CourseImage' className='w-full md:w-[35%] object-cover rounded-md'/>
                                        <div className='flex flex-col gap-2'>
                                            <p className='text-base md:text-xl'>{course?.name}:</p>
                                            <p className='text-richblack-200 text-sm md:text-base'>{course?.description}</p>
                                            <p className='text-richblack-25 text-sm'>Created: {formatDate(course?.createdAt)}</p>
                                            <p>
                                                {
                                                    course?.status === COURSE_STATUS.DRAFT ? (
                                                        <div className='w-fit text-xs md:text-sm bg-richblack-700 rounded-full py-1 px-3 flex flex-row items-center justify-center text-pink-100 gap-1'><LuClock9/>Draft</div>
                                                    ) : (
                                                        <div className='w-fit text-xs md:text-sm bg-richblack-700 rounded-full py-1 px-3 flex flex-row items-center justify-center text-yellow-25 gap-1'><MdVerified/>Published</div>
                                                    )
                                                }
                                            </p>
                                        </div>
